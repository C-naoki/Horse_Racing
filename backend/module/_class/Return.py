...
class Return:
        ...
        for place, race_id_place in race_id_dict.items():
            for kai, race_id_kai in race_id_place.items():
                ...
                for race_id in race_id_kai:
                    pbar.update(1)
                    pbar.set_description("scrape  return table in {} {}回".format(
                        [k for k, v in place_dict.items() if v == str(place).zfill(2)][0], kai
                    ))
    ...
    def fukusho(self):
        ...
        returns.columns = ['return_1', 'return_2', 'return_3']
